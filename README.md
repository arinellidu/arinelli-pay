...
*One real loop, no mocks: a registry row corrected in place → that CPF promoted to client → contract → invoice → Pix QR (valid EMV) → simulated payment → HMAC-verified webhook → outbox → Go worker → invoice settles live via 3s polling. The gold settlement sweep is driven by that same confirmed event — it never simulates success.*

Recorded by `apps/web/scripts/demo-gif.mjs` against the full local stack. It provisions one synthetic person, then fixes that record from the row itself and creates the client, contract, invoice and Pix charge through the UI, so the portfolio film is rerunnable from scratch. `apps/web/e2e/demo.spec.ts` independently asserts the settlement loop.
